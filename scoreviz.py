...
def mincostviz(args):
    ...
    try:
        ...
        url = "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/madrid?unitGroup=metric&elements=datetime%2CdatetimeEpoch%2Ctemp%2Chumidity&include=fcst%2Ccurrent&key=TYBG6ANKGHCLAXPV53MLV4WJ2&contentType=json"
        response = requests.request("GET", url)
        t=response.json()['currentConditions']
        pointExt=[(t['temp'],t['humidity'])]
        
    for ig in range(len(df.index)):
        sensorid=df["sensorid"][ig]
        temperature=float(df["temperature"][ig])
        humidity=float(df["humidity"][ig])
        try:
        ...
        for i in range(nh):
            for j in range(nt):
                pointsGrid.append((round(pointH[i],2),round(pointT[j],2)))
        ...
        for i in range(len(steps)):
            ...
            if i==0:
              output=str(pointsGrid[steps[i]])
            else:
              output=output+'->'+str(pointsGrid[steps[i]])
        if aviso ==0:
    ...
    return {'data': {'colnames': ['result'],'coltypes': ['STRING'],'rows': paths}}
